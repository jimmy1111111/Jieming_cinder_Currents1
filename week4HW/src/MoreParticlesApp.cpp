...
void MoreParticlesApp::mouseMove( MouseEvent event )
{
    ...
    for (int i = 0; i < NUM_PARTICLES; i++){
        angle =ci::randFloat(0,360);
        sinR= 50*sin (angle*PI/180) ;
        cosR = 50*cos(angle*PI/180);
        mParticles.push_back(Particle::create((glm::vec2)event.getPos() + glm::vec2(sinR,cosR), LIFE_SPAN, glm::vec2(ci::randFloat(-RANDOM_FORCE_RANGE, RANDOM_FORCE_RANGE), ci::randFloat(-RANDOM_FORCE_RANGE, RANDOM_FORCE_RANGE))));
...
void MoreParticlesApp::draw()
{
    ...
    for (auto& p: mParticles){
        ...
        gl::vertex(p->getPosition() + glm::vec2(norm.x, norm.y) * 0.2f);
    }
    ci::gl::end();
